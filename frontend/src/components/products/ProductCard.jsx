...
const ProductCard = ({ product, setSelectedProduct, setEnquiryOpen }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col rounded-[28px] overflow-hidden bg-white shadow-[0_18px_40px_rgba(0,0,0,0.10)] transition-all duration-250 hover:-translate-y-[6px] group ${isSold ? "opacity-90" : ""} lg:w-[320px] lg:h-[470px]`}
    >
      {/* 1. Image Section (Top) */}
      ...
      <div style={{ padding: "22px", display: "flex", flexDirection: "column", gap: "14px", flex: 1 }}>
        <Link to={`/products/${product.id}`}>
          <h3 style={{ fontSize: "20px", fontWeight: 900, lineHeight: 1.15, letterSpacing: "-0.4px", color: "#111827" }} className="font-display line-clamp-2">
            {product.name}
          </h3>
        </Link>
